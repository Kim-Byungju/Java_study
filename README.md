...
> 자바기반 풀스택 개발자 과정 국비지원 수업 및 책<혼자 공부하는 자바>에 기반한 자바 기록입니다. 처음부터 한번에 정리는 되지 않겠지만, 수채화를 덧칠 하듯이 계속해서 추가할 예정 입니다.
-----
### 📍Ch0 자바 개요
  ...
  2. 대입 연산자
   - 대입연산자는 변수에 값을 대입할 때 사용하는 이항 연산자이며, 피연산자들의 결합방향은 오른쪽에서 왼쪽이다.
     ...
     *= : 왼쪽의 피연산자에 오른쪽의 피연산자를 곱한 후,  그 결과값을 왼쪽의 피연산자에 대입함.
     ...
     ```
  3. 증감 연산자
   - 증감연산자는 피연산자를 1씩 증가, 감소시킬떄 사용하는 연산자이다. 피연산자가 하나뿐인 단항 연산자이다. 
   - 증감연산자는 해당 연산자가 피연산자의 어느쪽에 위치하는가에 따라 연산의 순서 및 결과가 달라진다.
     ```java
     ++x : 먼저 피연산자의 값을 1증가 시킨후에 해당 연산을 진행함.
     x++ : 먼저 해당연산을 수행하고 나서, 피연산자의 값을 1 증가시킴.
     --x : 먼저 피연산자의 값을 1 감소시킨 후에 해당연산을 진행함.
...
6. 향상된 for문
 ...
 7. 배열 복사
   - 배열의 복사는 for문으로 아래와 같은 방식으로 복사할수 있다.
     ...
     ```
   - 그렇지만, 간편하게 System.arraycopy()메소드를 이용하여 배열을 복하하는 방법이 있다. 
      ```java
